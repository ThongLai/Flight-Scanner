...
def add_wait_target(df: pd.DataFrame,
                    drop_threshold: float = 0.02) -> pd.DataFrame:
    """Label whether waiting ~7 days yields a lower price."""
    df = df.sort_values(["route", "departure_date", "collected_date"]).copy()
    grp = df.groupby(["route", "departure_date"], observed=True)["price"]
    df["price_future7"] = grp.shift(-7)
    df["should_wait"] = (
        df["price_future7"] < df["price"] * (1 - drop_threshold)).astype(int)
    return df.dropna(subset=["price_future7"])
